...
server <- function(input, output) {  
  ...
  gafl$weight <- with(gafl, ifelse(type == 'Economic', (40/21), 1))
  ...
  compass<- ggplot(gafl, mapping = aes(x = Economic, y = Social)) +
    annotate("rect", xmin = 40, xmax = 0, 
             ymin = 40, ymax = 0, fill= "#40acff", alpha = 0.5)  + 
    annotate("rect", xmin = -40, xmax = 0, 
             ymin = -40, ymax = 0 , fill= "#f97c7c", alpha = 0.5) + 
    annotate("rect", xmin = 0, xmax = 40, 
             ymin = 0, ymax = -40, fill= "#fdfd96", alpha = 0.5) + 
    annotate("rect", xmin = 0, xmax = -40, 
             ymin = 40, ymax = 0, fill= "#a0e7a0", alpha = 0.5) + 
    annotate("segment", x = 0, xend = 0, y = -40, yend = 40, color = 'black') +
    annotate("segment", x = -40, xend = 40, y = 0, yend = 0, color = 'black')+
    geom_point() + theme_bw() + geom_text(label=gafl$initials,
                                          nudge_x = 2, 
                                          nudge_y = 2) +
    ...
    scale_x_continuous(expand = c(0, 0), breaks = seq(-40,40,10), 
